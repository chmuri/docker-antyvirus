...
RUN sudo apt install apt-transport-https -yRUN echo 'Acquire::Languages "none";' | sudo tee /etc/apt/apt.conf.d/99disable-translations
RUN echo "deb https://packages.cisofy.com/community/lynis/deb/ stable main" | sudo tee /etc/apt/sources.list.d/cisofy-lynis.listRUN apt update
RUN apt install lynis  -yRUN apt-get install clamav-daemon -y
